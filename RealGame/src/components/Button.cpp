...
Button::Button(const glm::vec2& pos, const glm::vec2& size, const std::string& text, std::function<void(Game*)>callback) :position(pos),size(size),text(text),onClick(callback){}